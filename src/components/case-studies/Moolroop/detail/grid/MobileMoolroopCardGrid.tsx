...
import styles from './MobileMoolroopCardGrid.module.css';
...
/* Plain vertical stack, numeric order (no top/bottom row split — that's a
   ...
   touch has no hover. Tapping a card reuses the same MoolroopPanel used on
   desktop (open/close/prev/next), just triggered by tap instead of click. */
export default function MobileMoolroopCardGrid() {
  const [openIndex, setOpenIndex] = useState<number | null>(null);
  /* See MoolroopCardGrid: ±1 while navigating (directional slide), 0 while
      ...
      {CARDS.map((card, i) =>
        openIndex === i ? null : (
          /* Stable outer button — never remounts, so it never disturbs this
             ...
             until this card is eventually removed — see MoolroopCardFace)
